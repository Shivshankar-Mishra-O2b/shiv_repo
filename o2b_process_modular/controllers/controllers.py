...
#                 # self.create_menu1(activities,node_length,process,custom_model,process.get('process_name'),process_image,process.get('process_group'),activity_name,activity_type,node_id,record_process.id,process_token,activity_group)
#                 menu_sequence = 10
...
#             # self.create_menu(activities,node_length,process,custom_model,process.get('process_name'),process_image,process.get('process_group'))
...
#         # mdm menu or its action start here
#         # p_action = None
#         # p_menu_id = None
#         # counter = 1
#         # for rec in mdm:
#         #     if counter ==1:
#         #         action_name = 'MDMACTION' + rec.get('model').lower().replace('.','_').strip()
#         #         model = rec.get('model')
#         #         print("action name is : %s ", action_name)
#         #         p_action = request.env['ir.actions.act_window'].sudo().search(['&',('name','=',action_name),('res_model','=',model)],order="id desc" , limit=1)
#         #         if p_action:
#         #             counter +1
...
#     # CREATE MENU METHOD
#     def create_menu1(self,activities, node_length,process,custom_model,process_name,process_image,process_group,activity_name,activity_type,node_id,record_process,process_token,activity_group):
#         model_name = custom_model.model
#         main_app_menu_id = request.session.get('main_app_menu_id')
#         main_app_start_menu = request.session.get('main_app_start_menu')
#         if  activity_type =='start':
#             record_process_group_m = request.env['o2b.process.modular.group'].sudo().search([('process_name', '=',process_name),('activity_type','=','main app')])
#             record_process_group_s = request.env['o2b.process.modular.group'].sudo().search([('process_name', '=',process_name),('activity_type','=','start')])
#             record_process_group_start_app = request.env['o2b.process.modular.group'].sudo().search([('process_name', '=',process_name),('activity_type','=','start app')])
#             if record_process_group_m and record_process_group_s and record_process_group_m.action_id == record_process_group_s.action_id:
#                 if record_process_group_m.menu_id:
#                     pass
#                 else:
#                     result = self.save_image_from_buffer(process_image, process.get('process_name'))
#                     web_icon_path = ''
...
#                     # main app process related
#                     main_parrent_menu = request.env['ir.ui.menu'].sudo().search([('action', '=','ir.actions.act_window,'+ str(record_process_group_m.action_id)),('parent_id','=',False),('name', '=',process.get('process_name').strip())])
#                     if main_parrent_menu:
#                         main_parrent_menu.sudo().write({
#                             'name'      : process.get('process_name').strip(),
#                             'parent_id' : False,
#                             'action'    : 'ir.actions.act_window,'+ str(record_process_group_m.action_id),
#                             'web_icon'  : web_icon_path
#                             })
#                     else:
#                         main_parrent_menu = request.env['ir.ui.menu'].with_user(2).sudo().create({
...
#                             'web_icon'  : web_icon_path
#                             })
#                     record_process_group_m.sudo().write({
#                             'menu_id'   : str(main_parrent_menu.id),
#                             'parent_id' : 'null'
#                             })

#                     main_start_menu = request.env['ir.ui.menu'].sudo().search([('action', '=','ir.actions.act_window,'+ str(record_process_group_m.action_id)),('parent_id','=',main_parrent_menu.id),('name', '=',activity_name.strip())])
#                     if main_start_menu:
#                         main_start_menu.sudo().write({
#                             'name'      :  activity_name.strip(),
#                             'parent_id' :  main_parrent_menu.id,
#                             'action'    : 'ir.actions.act_window,'+ str(record_process_group_m.action_id),
#                             })
#                     else:
#                         main_start_menu = request.env['ir.ui.menu'].with_user(2).sudo().create({
#                             'name'      :  activity_name.strip(),
#                             'parent_id' :  main_parrent_menu.id,
#                             'action'    : 'ir.actions.act_window,'+ str(record_process_group_m.action_id),
#                             })
#                     record_process_group_start_app.sudo().write({
#                             'menu_id'   : str(main_start_menu.id),
#                             'parent_id' : str(main_parrent_menu.id),
#                             })
                   
#                     start_sub_menu = request.env['ir.ui.menu'].sudo().search([('action', '=','ir.actions.act_window,'+ str(record_process_group_m.action_id)),('parent_id','=', main_start_menu.id),('name', '=',activity_name.strip())])
#                     if start_sub_menu:
#                         start_sub_menu.sudo().write({
#                             'name'      :  activity_name.strip(),
#                             'parent_id' :  main_start_menu.id,
#                             'action'    : 'ir.actions.act_window,'+ str(record_process_group_m.action_id),
#                             })
#                     else:
#                         start_sub_menu = request.env['ir.ui.menu'].with_user(2).sudo().create({
...
#                             'action'    : 'ir.actions.act_window,'+ str(record_process_group_m.action_id),
#                             })
#                     record_process_group_s.sudo().write({
#                             'menu_id'   : str(start_sub_menu.id),
#                             'parent_id' : str(main_start_menu.id),
#                             })
#                     self.show_menu_group_wise(main_start_menu.id,activity_name,activity_group,'sub_menu',custom_model)
#                     request.session['main_app_menu_id'] = main_parrent_menu.id
#                     request.session['main_app_start_menu'] = main_start_menu.id
#                     request.session['main_app_start_sub_menu'] = start_sub_menu.id
             
#         if activity_type not in ['decision']:
#             main = request.session.get('main_app_menu_id')
#             start = request.session.get('main_app_start_menu')
#             start_sub = request.session.get('main_app_start_sub_menu')
#             if start_sub:
#                 record_process_group_m = request.env['o2b.process.modular.group'].sudo().search([('process_name', '=',process_name),('activity_name','=',activity_name),('activity_type','not in',['start app','main app'])])
#                 except_start_menu = request.env['ir.ui.menu'].sudo().search([('action', '=','ir.actions.act_window,'+ record_process_group_m.action_id),('parent_id','=', start_sub),('name', '=',activity_name.strip())])
#                 if except_start_menu:
#                     except_start_menu.sudo().write({
#                         'name'      : activity_name.strip(),
#                         'parent_id' :  start_sub,
#                         'action'    : 'ir.actions.act_window,'+ record_process_group_m.action_id,
#                         'web_icon'  : 'o2b_process_modular,static/description/icon.png'
#                         })
#                 else:
...
#                         })
#                 record_process_group_m.sudo().write({
#                         'menu_id'   : str(except_start_menu.id),
#                         'parent_id' : str(start_sub),
#                         })
#         self.initiate_access(custom_model.model)

#     # CREATE MENU METHOD
#     def create_menu(self,activities, node_length,process,custom_model,process_name,process_image,process_group):
#         model_name = custom_model.model
#         count = 0
#         parrent_record = request.env['ir.ui.menu'].sudo().search([('name', '=',process_name.strip())])
#         start_menu = ''
#         for i in range(node_length):
#             # if i==0 and activities[i].get('activity_type')!= 'end' and activities[i].get('activity_type')!= 'decision':
#             if i==0 and activities[i].get('activity_type')!= 'end':
#                 record_action = request.env['ir.actions.act_window'].sudo().search([('name', '=',activities[i].get('activity_name')),('res_model','=', model_name)],order="id desc" , limit=1)
#                 if parrent_record :
#                     parrent_sub_record = request.env['ir.ui.menu'].sudo().search([('name', '=',activities[i].get('activity_name').strip())])
#                     if parrent_sub_record:
#                         parrent_sub_record.with_user(2).sudo().write({
#                             'name'      : activities[i].get('activity_name').strip(),
#                             'parent_id' :  parrent_record.id,
#                             'action'    : 'ir.actions.act_window,'+ str(record_action.id),
#                             'web_icon'  : 'o2b_process_modular,static/description/icon.png'
#                             })
#                         start_menu = parrent_sub_record
#                     else:
#                         new = request.env['ir.ui.menu'].with_user(2).sudo().create({
#                         'name'      : activities[i].get('activity_name').strip(),
#                         'parent_id' :  parrent_record.id,
#                         'action'    : 'ir.actions.act_window,'+ str(record_action.id),
#                         'web_icon'  : 'o2b_process_modular,static/description/icon.png'
...
#                 else:
#                     parrent_record.unlink()
#                     # call web icon creation
#                     result = self.save_image_from_buffer(process_image, process.get('process_name'))
#                     web_icon_path = ''
#                     if result:
#                         web_icon_path = 'o2b_process_modular,' + 'static/description/' +process_name.replace(' ','_').lower() + '/icon.png'
...
#                     if record_action:
#                         parrent_record_new = request.env['ir.ui.menu'].with_user(2).sudo().create({
#                             'name'      : process.get('process_name').strip(),
#                             'parent_id' : False,
#                             'action'    : 'ir.actions.act_window,'+ str(record_action.id),
#                             'web_icon'  : web_icon_path
#                             })
#                         record_menu = parrent_record_new
#                         parrent_record = parrent_record_new.id
#                         parrent_record_new1 = request.env['ir.ui.menu'].with_user(2).sudo().create({
#                             'name'      : activities[i].get('activity_name').strip(),
#                             'parent_id' :  parrent_record_new.id,
#                             'action'    : 'ir.actions.act_window,'+ str(record_action.id),
#                             })
#                         start_menu = parrent_record_new1
#                         parrent_record_new2 = request.env['ir.ui.menu'].with_user(2).sudo().create({
#                             'name'      : activities[i].get('activity_name').strip(),
#                             'parent_id' :  start_menu.id,
#                             'action'    : 'ir.actions.act_window,'+ str(record_action.id),
#                             })
#                         self.show_menu_group_wise(parrent_record_new1.id,activities[i].get('activity_name'),activities[i].get('activity_group'),'sub_menu',custom_model)
#                 count = count+1
#             elif activities[i].get('activity_type')  not in ['end','decision'] and activities[i].get('activity_type') is not None :
#                 parrent_sub_record = request.env['ir.ui.menu'].sudo().search([('name', '=',activities[i].get('activity_name').strip())])
#                 record_action = request.env['ir.actions.act_window'].sudo().search([('name', '=',activities[i].get('activity_name')),('res_model','=', custom_model.model)],order="id desc" , limit=1)
#                 new_menu = request.env['ir.ui.menu'].with_user(2).sudo().create({
#                 'name': activities[i].get('activity_name'),
#                 'parent_id': start_menu.id,
#                 'action': 'ir.actions.act_window,'+ str(record_action.id),
#                 })
#                 self.show_menu_group_wise(new_menu.id,activities[i].get('activity_name'),activities[i].get('activity_group'),'sub_menu',custom_model)
#             else:
#                 pass
#         for i in range(node_length):
#             if  activities[i].get('activity_type')== 'end':
#                 parrent_record = request.env['ir.ui.menu'].sudo().search([('name', '=',process.get('process_name').strip())])
#                 if parrent_record :
#                         record_menu = parrent_record_new
#                         parrent_record = parrent_record_new.id
#                         record_action = request.env['ir.actions.act_window'].sudo().search([('name', '=',activities[i].get('activity_name')),('res_model','=', custom_model.model)],order="id desc" , limit=1)
